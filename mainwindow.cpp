	...
	if (fileopener.exec())
	{
		...
		if (!imageMutator.loadSVG(path))
		{
			QMessageBox::critical(this, tr("File Read Error"),
					tr("Errored while loading image"));
			return;
		}
		updateCurrentImage();
		updateStatus();
...
void MainWindow::saveImage()
{
	...
	if (fileopener.exec())
	{
		...
		{
			if (!imageMutator.saveBestImage(path +
						(path.endsWith(QString(".png"), Qt::CaseInsensitive) ? "" : ".png")))
			{
				QMessageBox::critical(this, tr("File Write Error"),
						tr("Errored while saving image"));
			}
		}
		else
		{
			if (!imageMutator.saveSVG(path +
						(path.endsWith(QString(".svg"), Qt::CaseInsensitive) ? "" : ".svg")))
			{
				QMessageBox::critical(this, tr("File Write Error"),
						tr("Errored while saving image"));
			}
...
void MainWindow::autosave()
{
	...
	if (saveIncremental)
	{
		...
		imageMutator.saveBestImage(incrementalFilename +
				theTime.toString(QString("ddMMyyhhmmsszzz")) + QString(".png"));
	}
}
...
void MainWindow::updateStatus()
{
	QString report =
		"Mutation Count: " +
		QString::number(imageMutator.getGoodMutationCount()) + "/" +
		QString::number(imageMutator.getTestedMutationCount()) +
		"   Score: " + QString::number(imageMutator.getCurrentScore()) + "%" +
		"   Number of Polygons: " +
		QString::number(imageMutator.getNumberOfPolygons()) +
...
void MainWindow::updateCurrentImage()
{
	...
	if (imageMutator.getCurrentImage().width() >
			imageMutator.getCurrentImage().height())
...
{
	QImage temp;
	if (imageMutator.getOriginalImage().width() >
			imageMutator.getOriginalImage().height())
